...
class NStepsAgent(QAgent):
    ...
    def simulate(self, policy, env, train_flag=False, eval_flag=False):
        ...
        while not done:
            ...
            if done: self.T = t + 1
            if not train_flag: self.store_transitions(state, action, reward,eval_flag=eval_flag)
            if train_flag and tau >= 0: self.update_vol2(state, action, new_state, reward, done,
                                                         tau=tau)  # TODO make simulate,update abstact for all agents?
            cum_reward += reward
            state = deepcopy(new_state)
            t += 1
        return cum_reward

    def update_vol2(self, state, action, new_state, reward, done,
                    tau):  # TODO make simulate,update abstact for all agents?
